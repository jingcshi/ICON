...
class SimCSE(object):
    ...
    def search(self, queries: Union[str, List[str]], 
        ...
        if not self.is_faiss_index:
            ...
            results = [(i, self.index["sentences"][idx], score) for idx, score in id_and_score]
            return results
        else:
            ...
            def pack_single_result(dist, idx):
                results = [(i, self.index["sentences"][i], s) for i, s in zip(idx, dist) if s >= threshold]
                return results
